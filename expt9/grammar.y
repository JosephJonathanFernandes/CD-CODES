...
while_stmt:
      ...
      '(' cond ')' '{' stmt_list '}' {
          ...
          char *exit = newlabel();
          /* $4 is cond (because of the mid-rule action shifting positions) */
          emit("if %s == 0 goto %s", $4, exit);
          /* body (stmt_list) already emitted during parsing */
          emit("goto %s", start);
          printf("%s:\n", exit);
          free(start); free(exit); free($4);
      }
    ;
